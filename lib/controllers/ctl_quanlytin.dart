...
class Ctl_Quanlytin extends GetxController{
  ...
  /// -----------------------------------------------------------------------------------------*
  final RxList<Map<String, dynamic>> _lstTinCT = <Map<String, dynamic>>[].obs;
  List<Map<String, dynamic>> get lstTinCT => _lstTinCT.value;
  ...
  onDeleteKhach(int KhachID) async {
    String strNgay = DateFormat("yyyy-MM-dd").format(_ngaylam.value);
    List<Map<String, dynamic>> tin = await db.loadData(tbName: 'TXL_TinNhan', condition: "KhachID = $KhachID AND Ngay = '$strNgay'");
    List<int> lstMaTin = tin.map((e) => int.parse(e['ID'].toString())).toList();
    if(lstMaTin.isNotEmpty){
      await db.deleteData(tbName: 'TXL_TinNhan',condition: "KhachID = $KhachID AND Ngay = '$strNgay'");
  ...
  onLoadTinCT(String ID) async{
    ...
    ''');
    /// -----------------------------------------------------------------------------------------*
    List<String> lstMien = data.map((e) => e['Mien'].toString()).toSet().toList();
    lstMien = sortMien(lstMien); /** Sắp xếp lại miên [N, T, B] **/
    /// -----------------------------------------------------------------------------------------*
  ...
  onDeleteTin(int ID,int KhachID)async{
    ...
    if(ID == Ctl_Xuly().to.matin) Ctl_Xuly().to.onLoadTinNhan(); /// Nếu xóa tin đang hiện thì load lại trang tính
  ...
  final RxBool _isLoading = false.obs; bool get isLoading => _isLoading.value;
  // List<TableRow> get lstXemCT => _lstXemCT.value;
  Rx<double>  _tongST = 0.0.obs; String get tongST => NumberFormat('#,###').format(_tongST.value).toString();
  Rx<double>  _tongXac = 0.0.obs; String get tongXac => NumberFormat('#,###').format(_tongXac.value).toString();
  Rx<double>  _tongVon = 0.0.obs; String get tongVon => NumberFormat('#,###').format(_tongVon.value).toString();
  Rx<double>  _tongTrung = 0.0.obs; String get tongTrung => NumberFormat('#,###').format(_tongTrung.value).toString();
  RxString tin = ''.obs;
  xemchitiet(String matin) async {
   _isLoading.value = true;
    clearXemCT();
    String? _tin = await db.dLookup('TinXL', 'TXL_TinNhanCT', 'TinNhanID = $matin');
    ...
    ''');
      if(data.isNotEmpty){
        lstXemCT.value = data.map((e){
          ...
          return {
            'Tin': e['Tin'].toString().replaceFirst('thang', 'da').replaceFirst('xien', 'dx')+e['SoTien'].toString().formatDouble,
            'Xac': NumberFormat('#,###').format(e['Xac']).toString(),
            'Von': NumberFormat('#,###').format(e['Von']).toString(),
            'Trung': e['TienTrung'] == 0.0 ? '' :  NumberFormat('#,###').format(e['TienTrung']).toString(),
            'SLT': e['SoLanTrung'] == 0.0 ? '' :  NumberFormat('#,###').format(e['SoLanTrung']).toString(),
            // 'i':e['i']??0
          };
        }).toList();
      }
  ...
  clearXemCT(){
    _tongST.value = 0.0; _tongXac.value = 0.0; _tongVon.value = 0.0;  _tongTrung.value = 0.0;
    tin.value = '';
    lstXemCT.clear();
  }
/// -----------------------------------------------------------------------------------------*
